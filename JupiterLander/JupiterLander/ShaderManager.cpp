...
mat4 ShaderManager::OrthoCam = mat4();
ShaderManager::ShaderManager() {}			 // Nothing to do in constructor
void ShaderManager::Clear() {			     // Need to delete programs 
...
GLuint ShaderManager::CreateShader(GLenum shaderType, const string& source, const string& shaderName) {
	...
	if (compile_result == GL_FALSE) {
		...
		cout << "ERROR compiling shader: " << shaderName << endl << &shader_log[0] << std::endl;
		return 0;
	}
	return shader;
}

mat4 ShaderManager::GetCam() {
	return OrthoCam;
}

void ShaderManager::SetCam(float dx, float dy) {
	OrthoCam.m[3][0] = dx; 
	OrthoCam.m[3][1] = dy;
}
...
void ShaderManager::CreateProgram(const string& shaderName, const string& vertexShaderFilename,
	...
	if (programs.count(shaderName) > 0) {
		cout << "A shader with the name, " << shaderName <<", already exists" << endl;
		return;
	}
